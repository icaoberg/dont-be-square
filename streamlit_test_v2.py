...
if st.button(f"Calculating FAIR scores") and search_form:
    try:
        ...
        if search_form == 'Lab':
            assay_types = utils.assay_types_for_lab(df, lab_selected)
            items = sorted(assay_types)
            st.divider()
            st.subheader(f"🧪 Assay Types related to this lab: ({len(items)})")
            n_cols = min(3, len(items)) or 1
            cols = st.columns(n_cols)
            for i, item in enumerate(items):
                cols[i % n_cols].markdown(f"• **{item}**")

        elif search_form == 'Assay type':
            labs = utils.labs_for_assay(df, assay_selected)
            items = sorted(labs)
            st.divider()
            st.subheader(f"🏛️ Labs associated with this assay type: ({len(items)})")
            n_cols = min(3, len(items)) or 1
            cols = st.columns(n_cols)
            for i, item in enumerate(items):
                cols[i % n_cols].markdown(f"• **{item}**")
