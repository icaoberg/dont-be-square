...
def assay_types_for_lab(df, lab_name):
    """Return unique assay types (dataset_type) for a given lab (group_name)."""
    try:
        df_lab = df[df['group_name'] == lab_name]
        return df_lab['dataset_type'].dropna().unique()
    except Exception:
        return np.array([])

def labs_for_assay(df, assay_type):
    """Return unique labs (group_name) that have a given assay type (dataset_type)."""
    try:
        df_assay = df[df['dataset_type'] == assay_type]
        return df_assay['group_name'].dropna().unique()
    except Exception:
        return np.array([])
